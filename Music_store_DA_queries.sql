...
SELECT SUM(total) as most_money,billing_city FROM Invoice
...
SELECT COUNT(t.name) as track_count,a.name FROM track t 
...
SELECT COUNT(a.artist_id) as track_count,a.name FROM track t 
JOIN album b ON t.album_id=b.album_id
JOIN artist a ON b.artist_id=a.artist_id
JOIN genre g ON t.genre_id=g.genre_id
	WHERE g.name LIKE 'Rock'
...
-- 8. Return all the track names that have a song length longer than the average song length. Return the 
